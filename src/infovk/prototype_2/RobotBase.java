...
public class RobotBase extends SimpleRobot implements Constants {
    ...
    /**
     * use {@link #setFire(double, RobotInfo)}
     */
    @Override
    @Deprecated
    ...
    public void setFire(double power, RobotInfo target) {
    ...
    /**
     *
     * use {@link #fireRelativeToEnergy(RobotInfo, double)}
     */
    @Deprecated
    protected void fireRelativeToEnergy(double baseVal) {
        setFire(Math.max(Math.min(baseVal * getEnergy() / energyPowerFactor, 4), 0.1));
    }

    /**
     *
     * use {@link #fireRelativeToEnergyAndDistance(RobotInfo, double, double)}
     */
    @Deprecated
    ...
    protected void fireRelativeToEnergy(RobotInfo target, double baseVal) {
        setFire(Math.max(Math.min(baseVal * getEnergy() / energyPowerFactor, Rules.MAX_BULLET_POWER), Rules.MIN_BULLET_POWER), target);
    }
    ...
    private final class BulletManager {
        ...
        private HashMap<String, Integer> mHitBullets;
        private HashMap<String, Integer> mMissedBullets;
        private HashMap<String, Integer> mShieldedBullets;
